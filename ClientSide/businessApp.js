(function(){
    ...
    $scope.announcements = {};
		...
		socket.on('receiveWordOfDay', function(data){
            if(data != null){
                $scope.wordOfDay.word = data.word;
				$scope.wordOfDay.word = $scope.wordOfDay.word.charAt(0).toUpperCase() + $scope.wordOfDay.word.slice(1);
                $scope.wordOfDay.def = data.def;
				$scope.wordOfDay.pos = data.pos.charAt(0).toUpperCase() + data.pos.slice(1);;
                $scope.$apply();
                console.log('Word of the day has been received');
            } else {
                console.log('Word of the day object is empty');
            }
        });
    });
