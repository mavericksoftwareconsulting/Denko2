...
var wordOfDay = { word: '', def: '', pos: '' };
var wordOfDayAPI = '76ea5cbda74c01447760b0bd03b0f739bcd37b6cfad92e30d';
...
var getWeather = function(startupCallback) {
    ...
    forecast.get(weatherAdminEdit.latitude, weatherAdminEdit.longitude, forecastOptions, function(err, res, data) {
        if (err) {
            setTimeout(function() {
                console.log('Unable to get weather. Trying again...' + err);
                getWeather(startupCallback);
            }, 1000 * errorCount);
            errorCount++;
            return;
        ...
        //Checking if the temp is over 80 degrees, then the icon changes.
        var checkIfHighTemp = function(weather) {
            if (weather.currently) {
                changeTemp(weather.currently);
            }
            if (weather.hourly) {
                for (var i = 0; i < 5; i++) {
                    changeTemp(weather.hourly[i]);
                }
            }
            if (weather.daily) {
                for (var i = 0; i < 5; i++) {
                    changeTemp(weather.daily[i]);
                }
            }
        }
        var changeTemp = function(timeOfDay) {
            if (timeOfDay.temperature) {
                var temp = timeOfDay.temperature.replace(/[^0-9]/g, '');
                if (temp > 79 && timeOfDay.image[0] == '/Images/WeatherIcons/clear-day.png') {
                    timeOfDay.image[0] = timeOfDay.image[0].slice(0, timeOfDay.image[0].length - 4) + "H" + timeOfDay.image[0].slice(timeOfDay.image[0].length - 4);
                }
            } else if (timeOfDay.alternating) {
                var temp = timeOfDay.alternating[2][0].replace(/[^0-9]/g, '');
                if (temp > 79 && timeOfDay.image[0] == '/Images/WeatherIcons/clear-day.png') {
                    timeOfDay.image[0] = timeOfDay.image[0].slice(0, timeOfDay.image[0].length - 4) + "H" + timeOfDay.image[0].slice(timeOfDay.image[0].length - 4);
                }
            }
    ...
    if (firstLoad) {
        firstLoad = false;
        setInterval(getWeather, 15 * 60000);
    }
};


//Get the word of the day
var getWordOfTheDay = function(startupCallback) {
    var validFeed = false;
    var req = new request('http://api.wordnik.com:80/v4/words.json/wordOfTheDay?api_key=' + wordOfDayAPI, function(error, response, body) {
        if (error) {
...
//Schedules word of the day refresh every 6 hours

    setTimeout(getWordOfTheDay, 72 * 300000);
...
(function() {
    ...
    var functions = [getFeeds, getCredentials, getContacts, getWeather, getAnnouncements, getBanner, getGorilla, getWordOfTheDay];
...
io.on('connection', function(socket) {
    ...
    socket.emit('receiveWordOfDay', wordOfDay);
    ...
    //Returns updated word of the day on user request
    socket.on('requestWordOfDay', function() {
        returnWordOfDay();
    });
    ...
    var returnWordOfDay = security.debounce(function() {
        socket.emit('receiveWordOfDay', wordOfDay);
    }, socket, socketTimeout, true);
